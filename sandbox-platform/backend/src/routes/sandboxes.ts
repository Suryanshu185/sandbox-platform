...
router.post(
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      const sandbox = await sandboxService.restartSandbox(
        req.auth!.userId,
        id!,
      );
...
router.delete(
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      const destroyed = await sandboxService.destroySandbox(
        req.auth!.userId,
        id!,
      );
...
router.post(
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      const { id } = req.params;
      const data = req.body
        ? ReplicateSandboxSchema.parse(req.body)
        : undefined;

      const newSandbox = await sandboxService.replicateSandbox(
        req.auth!.userId,
        id!,
        data,
      );

      await auditService.logSandboxReplicated(
        req.auth!.userId,
        id!,
        newSandbox.id,
      );
...
router.get(
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      const logs = await sandboxService.getLogs(
        req.auth!.userId,
        id!,
        Math.min(tail, 1000),
      );
...
router.get(
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      const containerId = await sandboxService.getContainerId(
        req.auth!.userId,
        id!,
      );
      if (!containerId) {
        res.status(400).json({
          ...
          error: {
            code: "NO_CONTAINER",
            message: "No container associated with sandbox",
          },
        });
        return;
      }
