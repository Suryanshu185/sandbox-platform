...
import { environmentService, NotFoundError, QuotaExceededError } from './EnvironmentService.js';
import * as docker from '../docker.js';
import type { Sandbox, SandboxStatus, SandboxPhase, PortMapping, SandboxResponse, SandboxLog } from '../types.js';
...
async function findAvailablePort(basePort: number, maxAttempts = 100): Promise<number> {
...
class SandboxService {
  ...
  async createSandbox(
    ...
    if (count >= MAX_SANDBOXES_PER_USER) {
      throw new QuotaExceededError(`Maximum ${MAX_SANDBOXES_PER_USER} concurrent sandboxes allowed per user`);
    }

    // Get environment and version
    const env = await environmentService.getEnvironment(userId, data.environmentId);
    if (!env) {
    ...
    await query<DbSandbox>(
      ...
      [sandboxId, userId, data.environmentId, versionId, sandboxName, JSON.stringify(ports), expiresAt]
    );

    // Create container in background
    this.provisionSandbox(sandboxId, userId, version, data.overrides?.env ?? {}).catch((err) => {
      logger.error({ err, sandboxId }, 'Failed to provision sandbox');
    });

    logger.info({ userId, sandboxId, environmentId: data.environmentId }, 'Sandbox creation initiated');
  ...
  private async provisionSandbox(
    ...
    try {
      // Get sandbox
      const sandbox = await queryOne<DbSandbox>('SELECT * FROM sandboxes WHERE id = $1', [sandboxId]);
      if (!sandbox) {
      ...
      await query('UPDATE sandboxes SET container_id = $1, phase = $2 WHERE id = $3', [
        containerId,
        'starting',
        sandboxId,
      ]);
      ...
      logger.info({ sandboxId, containerId }, 'Container started, waiting for healthy state...');
      ...
      if (healthy) {
        ...
        logger.error({ sandboxId, containerId, containerInfo }, 'Sandbox failed to become healthy');
        await query("UPDATE sandboxes SET status = 'error', phase = 'failed' WHERE id = $1", [sandboxId]);
      }
    } catch (err) {
      logger.error({ err, sandboxId }, 'Failed to provision sandbox');
      await query("UPDATE sandboxes SET status = 'error', phase = 'failed' WHERE id = $1", [sandboxId]);
  ...
  private async collectLogs(sandboxId: string, containerId: string): Promise<void> {
    try {
  ...
  // Get sandbox by ID
  async getSandbox(userId: string, sandboxId: string): Promise<SandboxResponse | null> {
    ...
    return this.toResponse(this.mapSandbox(row), logs.reverse().map((l) => `[${l.type}] ${l.text}`));
  }
  ...
  async startSandbox(userId: string, sandboxId: string): Promise<SandboxResponse> {
  ...
  async stopSandbox(userId: string, sandboxId: string): Promise<SandboxResponse> {
    const sandbox = await queryOne<DbSandbox>(
      'SELECT * FROM sandboxes WHERE id = $1 AND user_id = $2',
      [sandboxId, userId]
    );
  ...
  // Restart a sandbox
  async restartSandbox(userId: string, sandboxId: string): Promise<SandboxResponse> {
    const sandbox = await queryOne<DbSandbox>(
      'SELECT * FROM sandboxes WHERE id = $1 AND user_id = $2',
      [sandboxId, userId]
    );
  ...
  async replicateSandbox(
    ...
    const newName = data?.name ?? `${sandbox.name}-replica-${uuidv4().slice(0, 4)}`;
    ...
    if (!newPorts && sandbox.ports.length > 0) {
      ...
      logger.info({ originalPorts: sandbox.ports, newPorts }, 'Auto-assigned available ports for replica');
    }
  ...
  async expireSandboxes(): Promise<number> {
    ...
    for (const sandbox of expired) {
      try {
        ...
        await query("UPDATE sandboxes SET status = 'expired', phase = 'stopped', stopped_at = NOW() WHERE id = $1", [
          sandbox.id,
        ]);
        count++;
        logger.info({ sandboxId: sandbox.id }, 'Sandbox expired');
      } catch (err) {
        logger.error({ err, sandboxId: sandbox.id }, 'Failed to expire sandbox');
      }
    }
  ...
  async getContainerId(userId: string, sandboxId: string): Promise<string | null> {
    const sandbox = await queryOne<DbSandbox>(
  ...
  async syncStatus(sandboxId: string): Promise<void> {
    ...
    if (status !== sandbox.status || phase !== sandbox.phase) {
      await query('UPDATE sandboxes SET status = $1, phase = $2 WHERE id = $3', [status, phase, sandboxId]);
    }
  }
  ...
  private toResponse(sandbox: Sandbox, logsPreview?: string[]): SandboxResponse {
