...
export function InteractiveTerminal({ sandboxId, isRunning, token }: InteractiveTerminalProps) {
  ...
  useEffect(() => {
    ...
    const ws = new WebSocket(`${protocol}//${window.location.host}/ws/sandboxes/${sandboxId}/terminal?token=${token}`);
