...
import { listSandboxContainers, stopContainer, removeContainer } from './docker.js';
  ...
  try {
    ...
    for (const container of containers) {
      try {
        ...
        logger.info({ containerId: container.Id }, 'Container stopped and removed');
      } catch (err) {
        logger.error({ err, containerId: container.Id }, 'Failed to stop container');
      }
    }
