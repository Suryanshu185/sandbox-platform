  ...
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      const { environment, version } =
        await environmentService.createEnvironment(req.auth!.userId, data);

      await auditService.logEnvironmentCreated(
        req.auth!.userId,
        environment.id,
        {
          name: data.name,
...
router.put(
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      const { environment, version } =
        await environmentService.updateEnvironment(req.auth!.userId, id!, data);

      await auditService.logEnvironmentUpdated(
        req.auth!.userId,
        environment.id,
        {
...
router.post(
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      const result = await environmentService.setSecret(
        req.auth!.userId,
        id!,
        data.key,
        data.value,
...
router.delete(
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      const deleted = await environmentService.deleteSecret(
        req.auth!.userId,
        id!,
        key!,
      );
